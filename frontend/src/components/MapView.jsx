...
function MapView({ onSelectBuilding }) {
  useEffect(() => {
    ...
    L.tileLayer(
      "https://maps.geoapify.com/v1/tile/osm-bright/{z}/{x}/{y}.png?apiKey=6eab50851a0d47f7a26c013b266bedb6",
      { maxZoom: 19 }
    ).addTo(map);
      ...
      return L.divIcon({
        ...
        html: `
          <div class="number-pin-wrapper">
            <div class="number-marker">${num}</div>
            <div class="number-pin-tail"></div>
          </div>
        `,
        iconSize: [32, 42],
        iconAnchor: [16, 42],
    ...
    buildings.forEach((b) => {
      ...
      const icon = createNumberIcon(stats.total);
